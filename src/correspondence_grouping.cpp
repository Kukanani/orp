...
pcl::PointCloud<ORPPoint>::Ptr loadAnyType(std::string path) {
...
main (int argc, char *argv[])
{
  ...
  //  Load clouds
  //
  if(!(model = loadAnyType(model_filename_))) return -1;
  if(!(scene = loadAnyType(scene_filename_))) return -1;
